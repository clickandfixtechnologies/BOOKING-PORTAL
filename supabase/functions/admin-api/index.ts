...
async function technicians(db:any){const{data,error}=await db.from("technicians").select("*").order("full_name");if(error)throw error;return{technicians:data||[]};}
...
async function updateTechnician(db:any,input:any){if(!input?.id||!/^[0-9a-f-]{36}$/i.test(input.id)||!/^[6-9]\d{9}$/.test(String(input.mobile||"").replace(/\D/g,"")))throw new Error("Technician details are invalid.");const{data:existing,error:existingError}=await db.from("technicians").select("auth_user_id").eq("id",input.id).single();if(existingError||!existing)throw new Error("Technician not found.");let authUserId=existing.auth_user_id;if(!authUserId){const email=clean(input.email,254),password=String(input.password||"");if(!email||!password)throw new Error("A login email and password are required to activate this technician portal account.");if(password.length<12)throw new Error("Technician password must contain at least 12 characters.");const{data:authData,error:authError}=await db.auth.admin.createUser({email,password,email_confirm:true});if(authError)throw new Error(authError.message);authUserId=authData.user.id;}const{data,error}=await db.from("technicians").update({auth_user_id:authUserId,full_name:clean(input.full_name,120),mobile:String(input.mobile).replace(/\D/g,""),username:clean(input.username,80),specialization:Array.isArray(input.specialization)?input.specialization.slice(0,10):[],working_days:Array.isArray(input.working_days)?input.working_days:[1,2,3,4,5,6],working_start:input.working_start||"10:00",working_end:input.working_end||"19:00",is_active:input.is_active!==false}).eq("id",input.id).select().single();if(error)throw error;return{technician:data};}
